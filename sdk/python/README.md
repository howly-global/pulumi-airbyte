...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `howly-global` with the `ORG` of your provider.
   - replace all instances of the `github.com/howly-global/pulumi-airbyte` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-airbyte
  ...
  ignore: []
  ldflags:
  - -X github.com/howly-global/pulumi-airbyte/provider/pkg/version.Version={{.Tag}}
